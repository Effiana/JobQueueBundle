...
class CleanUpCommand extends Command
{
    ...
    private function findStaleJobs(EntityManager $em)
    {
        ...
        do {
            ...
            /** @var Job $job */
            $job = $em->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j
    ...
    {
        $incomingDepsSql = $con->getDatabasePlatform()->modifyLimitQuery("SELECT 1 FROM effiana_job_dependencies WHERE dest_job_id = :id", 1);
    ...
    private function resolveDependencies(EntityManager $em, Job $job)
    {
        ...
        $em->getConnection()->executeUpdate("DELETE FROM effiana_job_dependencies WHERE dest_job_id = :id", array('id' => $job->getId()));
    }
    ...
    private function findExpiredJobs(EntityManager $em, InputInterface $input)
    {
        $succeededJobs = function(array $excludedIds) use ($em, $input) {
            return $em->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.closedAt < :maxRetentionTime AND j.originalJob IS NULL AND j.state = :succeeded AND j.id NOT IN (:excludedIds)")
        ...
        $finishedJobs = function(array $excludedIds) use ($em, $input) {
            return $em->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.closedAt < :maxRetentionTime AND j.originalJob IS NULL AND j.id NOT IN (:excludedIds)")
        ...
        $canceledJobs = function(array $excludedIds) use ($em, $input) {
            return $em->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.state = :canceled AND j.createdAt < :maxRetentionTime AND j.originalJob IS NULL AND j.id NOT IN (:excludedIds)")
                ->setParameter('maxRetentionTime', new \DateTime('-'.$input->getOption('max-retention')))
