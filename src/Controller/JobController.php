...
namespace Effiana\JobQueueBundle\Controller;
...
class JobController extends Controller
{
    ...
    /**
     * @Route("/{id}", name = "effiana_job_details")
     */
    public function detailsAction(Job $job)
    {
        ...
        if ($this->getParameter('effiana_job_queue.statistics')) {
            $dataPerCharacteristic = array();
            foreach ($this->get('doctrine')->getManagerForClass(Job::class)->getConnection()->query("SELECT * FROM effiana_job_statistics WHERE job_id = ".$job->getId()) as $row) {
    ...
    /**
     * @Route("/{id}/retry", name = "effiana_job_retry_job")
     */
    public function retryJobAction(Job $job)
    {
        ...
        $url = $this->generateUrl('effiana_job_details', array('id' => $retryJob->getId()));
    ...
    {
        return $this->get('effiana_job_queue.job_manager');
    }
}
