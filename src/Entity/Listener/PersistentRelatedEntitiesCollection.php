...
class PersistentRelatedEntitiesCollection implements Collection, Selectable
{
    ...
    private function initialize()
    {
        ...
        $con = $this->registry->getManagerForClass('EffianaJobQueueBundle:Job')->getConnection();
        $entitiesPerClass = array();
        $count = 0;
        foreach ($con->query("SELECT related_class, related_id FROM effiana_job_related_entities WHERE job_id = ".$this->job->getId()) as $data) {
            $count += 1;
