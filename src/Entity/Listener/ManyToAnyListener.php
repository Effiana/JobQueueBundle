...
class ManyToAnyListener
{
    ...
    public function postLoad(\Doctrine\ORM\Event\LifecycleEventArgs $event)
    {
        ...
        if ( ! $entity instanceof \Effiana\JobQueueBundle\Entity\Job) {
            return;
        }
    ...
    public function preRemove(LifecycleEventArgs $event)
    {
        ...
        $con->executeUpdate("DELETE FROM effiana_job_related_entities WHERE job_id = :id", array(
            'id' => $entity->getId(),
        ));
    }
    ...
    public function postPersist(\Doctrine\ORM\Event\LifecycleEventArgs $event)
    {
        ...
        foreach ($this->ref->getValue($entity) as $relatedEntity) {
            ...
            $con->executeUpdate("INSERT INTO effiana_job_related_entities (job_id, related_class, related_id) VALUES (:jobId, :relClass, :relId)", array(
                'jobId' => $entity->getId(),
                'relClass' => $relClass,
    ...
    public function postGenerateSchema(\Doctrine\ORM\Tools\Event\GenerateSchemaEventArgs $event)
    {
        ...
        $table->addForeignKeyConstraint('effiana_job', array('job_id'), array('id'));
    }
}