...
namespace Effiana\JobQueueBundle\Entity\Repository;
...
use Effiana\JobQueueBundle\Retry\ExponentialRetryScheduler;
use Effiana\JobQueueBundle\Retry\RetryScheduler;
...
class JobManager
{
    ...
    {
        return $this->getJobManager()->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.command = :command AND j.args = :args")
    ...
    public function getOrCreateIfNotExists($command, array $args = array())
    {
        ...
        $firstJob = $this->getJobManager()->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.command = :command AND j.args = :args ORDER BY j.id ASC")
             ->setParameter('command', $command)
    ...
    private function acquireLock($workerName, Job $job)
    {
        $affectedRows = $this->getJobManager()->getConnection()->executeUpdate(
            "UPDATE effiana_job SET workerName = :worker WHERE id = :id AND workerName IS NULL",
            array(
                'worker' => $workerName,
                'id' => $job->getId(),
            )
    ...
    public function findAllForRelatedEntity($relatedEntity)
    {
        ...
        $rsm->addRootEntityFromClassMetadata('EffianaJobQueueBundle:Job', 'j');

        return $this->getJobManager()->createNativeQuery("SELECT j.* FROM effiana_job j INNER JOIN effiana_job_related_entities r ON r.job_id = j.id WHERE r.related_class = :relClass AND r.related_id = :relId", $rsm)
    ...
    public function findJobForRelatedEntity($command, $relatedEntity, array $states = array())
    {
        ...
        $sql = "SELECT j.* FROM effiana_job j INNER JOIN effiana_job_related_entities r ON r.job_id = j.id WHERE r.related_class = :relClass AND r.related_id = :relId AND j.command = :command";
    ...
    public function findPendingJob(array $excludedIds = array(), array $excludedQueues = array(), array $restrictedQueues = array())
    {
        ...
        $qb->select('j')->from('EffianaJobQueueBundle:Job', 'j')
    ...
    public function findIncomingDependencies(Job $job)
    {
        ...
        return $this->getJobManager()->createQuery("SELECT j, d FROM EffianaJobQueueBundle:Job j LEFT JOIN j.dependencies d WHERE j.id IN (:ids)")
    ...
    public function getIncomingDependencies(Job $job)
    {
        ...
        return $this->getJobManager()->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.id IN (:ids)")
                    ->setParameter('ids', $jobIds)
                    ->getResult();
    }
    ...
    private function getJobIdsOfIncomingDependencies(Job $job)
    {
        $jobIds = $this->getJobManager()->getConnection()
            ->executeQuery("SELECT source_job_id FROM effiana_job_dependencies WHERE dest_job_id = :id", array('id' => $job->getId()))
    ...
    {
        return $this->getJobManager()->createQuery("SELECT j FROM EffianaJobQueueBundle:Job j WHERE j.state IN (:errorStates) AND j.originalJob IS NULL ORDER BY j.closedAt DESC")
    ...
    {
        $queues =  $this->getJobManager()->createQuery("SELECT DISTINCT j.queue FROM EffianaJobQueueBundle:Job j WHERE j.state IN (:availableStates)  GROUP BY j.queue")
    ...
    {
        $result = $this->getJobManager()->createQuery("SELECT j.queue FROM EffianaJobQueueBundle:Job j WHERE j.state IN (:availableStates) AND j.queue = :queue")
            ->setParameter('availableStates', array(Job::STATE_RUNNING, Job::STATE_NEW, Job::STATE_PENDING))
